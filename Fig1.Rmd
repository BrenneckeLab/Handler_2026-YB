...
p= PLOTtable %>%
  pivot_longer(cols = c(`TTnorm_sRNAdata_average-WT`,`RNAnorm_sRNAdata_average-WT`), names_to = "NORMtype", values_to = "COUNT")%>%
  ggplot(aes(x=TYPE,y=COUNT,colour = TYPE, shape=CLUSTER))+
  # geom_point()+
  geom_quasirandom_rast( aes(size=dotSIZE), alpha=1, varwidth = TRUE, groupOnX = TRUE )+
  scale_y_log10(limits=c(0.001,300))+
  # scale_color_igv()+
  scale_color_manual(values = c("CDS" = "#0274b3", "UTR" = "#343991" , "CLUSTER" = "#e6a025" )) +  # Add this line
  scale_size_identity()+
  scale_shape_manual(values = c("other-source-loci" = 19, "flam" = 15, "20A" = 17, "77B" = 18))+
  stat_summary(
    aes(group = TYPE),
    fun = median, 
    geom = "crossbar", 
  ...
  stat_summary(
    fun = median,
    geom = "text",
    aes(group = TYPE, label = sprintf("%.2f", after_stat(y))),
    vjust = -0.5,
    size = 3,
    color = "red"
  )+  coord_cartesian(clip = "off")+
...
print(p)
ggsave("biogenesis-efficiency.clusterSPLIT.pdf", p, width = 4.3, height = 5, units = "in", dpi = 300)
...
## compare piRNA biogenesis efficiency with DGE
```{r biogenesis-factor screening - compare piRNA biogenesis efficiency with DGE, echo=TRUE, message=FALSE, warning=FALSE, fig.width=5, fig.height=7}
...
mutate(
across(
starts_with(c("sRNAdata_")),~.x / TTseq_RPKM, 
.names = "TTnorm_{.col}"),
across(
starts_with(c("sRNAdata_")),
~.x / RNAseq_RPKM,
...
BINdata = TABLEfilt %>%
select(FBgn, TYPE, `TTnorm_sRNAdata_average-WT`,`RNAnorm_sRNAdata_average-WT`,`TTnorm_sRNAdata_average-YB`,`RNAnorm_sRNAdata_average-YB`,`TTnorm_sRNAdata_average-ARMI`,`RNAnorm_sRNAdata_average-ARMI` )%>%  filter(TYPE == "UTR") %>%
  mutate(
    BINwt = cut_interval(log10(`TTnorm_sRNAdata_average-WT`), n = 4)
  )

DGE_YB = read_tsv("DGE_siLuc_vs_siYB_PA.txt")
IDconversion = read_tsv("fbgn_annotation_ID_fb_2026_02.tsv")

DGE_YB_FBgn = left_join(DGE_YB, IDconversion, by=join_by("gene_id" == "ID"))%>%
filter(! is.na(FBgn))%>%
filter(baseMean >= 10) %>%      # or another sensible cutoff, e.g. from a MA-plot inspection
select(FBgn, log2FoldChange)%>%
#rename log2FoldChange column
rename(log2FoldChange_YB = log2FoldChange)

TABLEmerged = left_join(BINdata, DGE_YB_FBgn, by="FBgn")

p = TABLEmerged %>%
# pivot_longer(cols = starts_with("log2FoldChange"), names_to = "LIB", values_to = "lob2FoldChange")%>%
ggplot(aes( x=`RNAnorm_sRNAdata_average-WT`, log2FoldChange_YB ))+
geom_point_rast(size=0.3, alpha=0.3,color="#343991")+
geom_smooth(data=. %>% filter(`RNAnorm_sRNAdata_average-WT`>0.01), aes(y=log2(`RNAnorm_sRNAdata_average-YB`/`RNAnorm_sRNAdata_average-WT`)),color="#343991")+
scale_x_log10()+
# facet_col(~LIB)+
coord_cartesian(clip = "off")+
...
ggsave("biogenesis-eff_vs_RNA-DGE.pdf", p, width = 3, height = 6)
